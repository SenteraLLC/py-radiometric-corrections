...
class CorrectImagesApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Radiometric Corrections for Sentera Sensors")
        self.geometry("600x500")
        self.create_widgets()

    def create_widgets(self):
        row = 0
        tk.Label(self, text="Input Path (folder)").grid(row=row, column=0, sticky="w")
        self.input_path_var = tk.StringVar()
        tk.Entry(self, textvariable=self.input_path_var, width=50).grid(
            row=row, column=1
        )
        ...
        tk.Label(self, text="Calibration ID").grid(row=row, column=0, sticky="w")
        self.calibration_id_var = tk.StringVar(value="CAL")
        tk.Entry(self, textvariable=self.calibration_id_var).grid(row=row, column=1)
        row += 1

        tk.Label(self, text="Output Path (folder)").grid(row=row, column=0, sticky="w")
        self.output_path_var = tk.StringVar()
        tk.Entry(self, textvariable=self.output_path_var, width=50).grid(
            row=row, column=1
        )
        ...
        tk.Entry(self, textvariable=self.exiftool_path_var, width=50).grid(
            row=row, column=1
        )
    ...
    def run_correction(self):
        ...
        if not input_path:
            ...
            return
        cmd = ["python", SCRIPT_PATH, input_path]
        ...
        def run_subprocess():
            try:
                ...
                for line in iter(process.stdout.readline, ""):
                    if line:
                        self.output_text.after(0, self.output_text.insert, tk.END, line)
                process.stdout.close()
                # After process ends, read any remaining stderr
                stderr = process.stderr.read()
                process.stderr.close()
                if stderr:
                    self.output_text.after(
                        0, self.output_text.insert, tk.END, "\nErrors:\n" + stderr
                    )
