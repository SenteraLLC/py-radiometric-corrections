...
def test_aruco_marker_detection():
    ...
    for marker_id in [23, 63, 217]:
        ...
        assert marker_id in ids, f"Aruco marker {marker_id} not detected. Found: {ids.flatten().tolist()}"
...
def test_aruco_marker_coefficient_selection():
    ...
    assert len(zenith_co.sg3144_batch1_coefficients) > 250, "Batch1 coefficients not loaded properly"
    assert len(zenith_co.sg3144_batch2_coefficients) > 250, "Batch2 coefficients not loaded properly"
    assert len(zenith_co.sg3144_batch3_coefficients) > 250, "Batch3 coefficients not loaded properly"
    
    # Verify they are lists, not tuples (no trailing comma issue)
    assert isinstance(zenith_co.sg3144_batch1_coefficients, list), "Batch1 should be a list"
    assert isinstance(zenith_co.sg3144_batch2_coefficients, list), "Batch2 should be a list"
    assert isinstance(zenith_co.sg3144_batch3_coefficients, list), "Batch3 should be a list"
    
    # Verify the first few non-zero values match expected batch data
    # Batch1 at index 250 should be 0.1239909
    assert abs(zenith_co.sg3144_batch1_coefficients[250] - 0.1239909) < 0.00001, "Batch1 coefficient mismatch"
    
    # Batch2 at index 250 should be 0.13493395
    assert abs(zenith_co.sg3144_batch2_coefficients[250] - 0.13493395) < 0.00001, "Batch2 coefficient mismatch"
    
    # Batch3 at index 250 should be 0.11927114
    assert abs(zenith_co.sg3144_batch3_coefficients[250] - 0.11927114) < 0.00001, "Batch3 coefficient mismatch"
